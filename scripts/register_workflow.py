...
def create_secret_payload(workflow_data):
    ...
    if "ComputeServers" in payload:
        for server_key, server_config in payload["ComputeServers"].items():
            ...
            if faas_type == "Lambda":
                ...
                if (
                    "AccessKey" in server_config
                    and server_config["AccessKey"] == f"{server_key}_ACCESS_KEY"
                ):
                ...
                if (
                    "SecretKey" in server_config
                    and server_config["SecretKey"] == f"{server_key}_SECRET_KEY"
                ):
                ...
                if (
                    "Token" in server_config
                    and server_config["Token"] == f"{server_key}_TOKEN"
                ):
                ...
                if (
                    "API.key" in server_config
                    and server_config["API.key"] == f"{server_key}_API_KEY"
                ):
    ...
    if "DataStores" in payload:
        for store_key, store_config in payload["DataStores"].items():
            # Replace placeholder values with actual credentials
            if (
                "AccessKey" in store_config
                and store_config["AccessKey"] == f"{store_key}_ACCESS_KEY"
            ):
                if (
                    store_key == "My_Minio_Bucket"
                    and credentials["My_Minio_Bucket_ACCESS_KEY"]
                ):
                    store_config["AccessKey"] = credentials[
                        "My_Minio_Bucket_ACCESS_KEY"
                    ]
            if (
                "SecretKey" in store_config
                and store_config["SecretKey"] == f"{store_key}_SECRET_KEY"
            ):
                if (
                    store_key == "My_Minio_Bucket"
                    and credentials["My_Minio_Bucket_SECRET_KEY"]
                ):
...
def deploy_to_github(workflow_data):
    ...
    try:
        ...
        vars = {
            f"{json_prefix.upper()}_PAYLOAD_REPO": f"{repo_name}/{workflow_data['_workflow_file']}"
        }
        ...
        for action_name, action_data in github_actions.items():
            ...
            container_image = workflow_data.get("ActionContainers", {}).get(
                action_name, "ghcr.io/faasr/github-actions-tidyverse"
            )
...
"""
            ...
            try:
                ...
                if existing_content.strip() == workflow_content.strip():
                    print(
                        f"File {workflow_path} content is already up to date, skipping update"
                    )
                else:
    ...
    for action_name, action_data in lambda_actions.items():
        try:
            ...
            if not container_image:
                ...
                print(
                    f"No container specified for action '{action_name}', using default: {container_image}"
                )
            ...
            environment_vars = {"SECRET_PAYLOAD": secret_payload}
            ...
            if "RequestEntityTooLargeException" in str(e):
                print(
                    f"Payload too large. SECRET_PAYLOAD size: {len(secret_payload)} bytes"
                )
